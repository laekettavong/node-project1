    ...
    // Optional data: none
     post : (data, callback) => {
        ...
        if(phone && password){
            ...
            _performance.mark('begin user look up');
            FsHelper.read(Endpoints.USERS, phone, (err, userData) => {
                _performance.mark('user look up complete');
                if(!err && userData){
                    ...
                    if(CryptoUtils.isAuthenticated(hashedPassword, userData.hashedPassword)){
                        ...
                        _performance.mark('complete token creation');
                        ...
                        FsHelper.create(Endpoints.TOKENS, tokenId, tokenObj, (err) => {
                            ...
                            _performance.measure('Begin to end', 'entered function', 'storing token complete');
                            _performance.measure('Validating user input', 'entered function', 'inputs validated');
                            _performance.measure('User look up', 'begin user look up', 'user look up complete');
                            _performance.measure('Password hashinbg', 'begin password hashing', 'password hashing complete');
                            _performance.measure('Token data creating', 'creating data for token', 'complete token creation');
                           ...
                           measurements.forEach((measurement) => {
                                debug(Colors.YELLOW, `${measurement.name} ${measurement.duration}`);
                           });
                    ...
                    FsHelper.read(Endpoints.USERS, phone, (err, userData) => {
                        if(!err && userData){
                            ...
                            if(userChecks.length < Config.maxChecks){
                                ...
                                // Verify that the give URL has DNS entries and therefore can resolve
                                let parsedUrl = _url.parse(`${protocol}://${url}`, true);
                                let hostName = StringUtils.isValidString(parsedUrl.hostname);                             
                                dns.resolve(hostName, (err, records) => {
                                    if(!err && records) {
                                        ...
                                        callback(400, {'Error' : ' The hostname of the specified URL did not resolve to any DNS entries'});
                                    }
                                });
                            } else {
